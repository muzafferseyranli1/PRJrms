...
import { MoreVertical, CheckCheck, Clock, Download, FileText, CheckSquare, Calendar, User, CornerDownRight } from 'lucide-react';
...
interface Props {
  ...
  onOpenContextMenu: (e: React.MouseEvent, msg: MessageItemType) => void;
...
export default function MessageItem({
  ...
  if (isSystem) {
    return (
      <div id={`msg-${message.id}`} className="flex justify-center my-3 transition duration-300">
        <div className="px-4 py-1.5 rounded-full bg-[#111b21]/90 border border-[#222e35] text-[11px] text-[#8696a0] flex items-center gap-1.5 shadow-sm">
  ...
  return (
    <div
      ...
      className={`group flex items-end gap-2 my-1 px-2 transition duration-300 ${
    ...
    >
      ...
      {!isMine && (
        <img
          ...
          className="w-7 h-7 rounded-full bg-[#202c33] object-cover flex-shrink-0 mb-1 border border-[#2a3942]"
        />
      )}
      ...
      <div className="relative max-w-[85%] sm:max-w-[70%]">
        {/* Bubble */}
        <div
          className={`rounded-2xl px-3.5 py-2 shadow-md relative ${
            isMine
        ...
        >
          ...
          {!isMine && (
            <p className="text-[11px] font-semibold text-[#00a884] mb-1 flex items-center gap-1.5">
              <span>{message.sender.fullName}</span>
              {message.sender.role === 'ADMIN' && (
                <span className="text-[9px] px-1.5 py-0.2 rounded bg-purple-500/20 text-purple-400 font-normal">
                  Yönetici
                </span>
              )}
            </p>
          ...
          {message.replyTo && (
            <div
              ...
              className={`mb-2 p-2 rounded-xl text-xs cursor-pointer border-l-4 transition flex items-start gap-2 ${
                isMine
                  ? 'bg-[#025142] border-[#00a884]/80 text-[#e9edef]'
                  : 'bg-[#111b21] border-[#00a884] text-[#e9edef]'
              }`}
            >
              <CornerDownRight className="w-3.5 h-3.5 text-[#00a884] flex-shrink-0 mt-0.5" />
              ...
              {message.attachments.map((att) => {
                ...
                if (isImage) {
                  return (
                    <div
                      ...
                      className="overflow-hidden rounded-xl cursor-pointer hover:opacity-90 transition border border-black/20 max-w-sm"
                    >
                      <img
                        src={att.fileUrl}
                        alt={att.fileName}
                ...
                return (
                  <a
                    ...
                    className="flex items-center gap-2.5 p-2 rounded-xl bg-black/20 hover:bg-black/30 border border-white/10 transition text-left group"
                  >
                    ...
                    <Download className="w-4 h-4 text-[#8696a0] group-hover:text-[#00a884] flex-shrink-0" />
                  </a>
                );
              })}
            </div>
          ...
          {message.task && (
            <div className="mt-2.5 p-2.5 rounded-xl bg-[#111b21]/80 border border-[#2a3942] backdrop-blur-sm">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5">
                  <CheckSquare className="w-4 h-4 text-[#00a884]" />
                  <span className="text-xs font-bold text-[#e9edef] truncate">{message.task.title}</span>
                </div>
                ...
                <select
                  ...
                  className={`text-[10px] font-semibold rounded-lg px-2 py-0.5 border focus:outline-none transition ${
              ...
              {/* Task Details Pill */}
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] text-[#8696a0] pt-1.5 border-t border-[#2a3942]/60">
                <span className="flex items-center gap-1">
                  <User className="w-3 h-3 text-[#00a884]" />
                  <span className="text-white font-medium">{message.task.assignedTo?.fullName || 'Atanmamış'}</span>
                </span>
                ...
                <span className="px-1.5 py-0.2 rounded text-[9px] bg-[#202c33] text-[#8696a0]">
          ...
          <div className="flex items-center justify-end gap-1 mt-1 text-[10px] text-[#8696a0]">
            ...
            {isMine && <CheckCheck className="w-3.5 h-3.5 text-[#00a884]" />}
          </div>
        </div>
        ...
        <button
          ...
          className="absolute top-1.5 right-2 opacity-0 group-hover:opacity-100 p-1 rounded-full bg-black/40 text-white hover:bg-black/60 transition shadow"
          ...
          <div className="flex flex-wrap items-center gap-1 mt-1">
            {Object.entries(reactionGroups).map(([emoji, data]) => (
              <button
                ...
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border transition shadow-sm ${
                  data.hasReacted
