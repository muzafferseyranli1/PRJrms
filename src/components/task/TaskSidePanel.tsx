...
export default function TaskSidePanel({
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'URGENT':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-red-500/20 text-red-400 border border-red-500/30">Acil</span>;
      case 'HIGH':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-orange-500/20 text-orange-400 border border-orange-500/30">Yüksek</span>;
      case 'LOW':
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-500/20 text-blue-400 border border-blue-500/30">Düşük</span>;
      default:
        return <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-500/20 text-amber-400 border border-amber-500/30">Normal</span>;
    }
  };

  return (
    <aside className="w-80 md:w-96 h-full flex flex-col bg-[#111b21] border-l border-[#222e35] select-none z-20 shadow-2xl animate-in slide-in-from-right duration-200">
      {/* Panel Header */}
      <div className="h-16 px-4 flex items-center justify-between bg-[#202c33] border-b border-[#222e35]">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              Grup Görevleri
              <span className="text-xs px-2 py-0.5 rounded-full bg-[#202c33] text-[#00a884] font-bold border border-[#2a3942]">
                {tasks.length}
              </span>
            </h3>
        ...
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg text-[#8696a0] hover:text-white hover:bg-[#111b21] transition"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="p-3 bg-[#111b21] space-y-2 border-b border-[#222e35]">
        <input
          ...
          className="w-full bg-[#202c33] border border-[#2a3942] rounded-xl px-3 py-1.5 text-xs text-[#e9edef] placeholder-[#8696a0]/60 focus:outline-none focus:border-[#00a884]"
        />
          ...
          filteredTasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  onClick={() => onScrollToMessage(task.messageId)}
